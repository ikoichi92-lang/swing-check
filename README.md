...
- ✅ **任意ライン描画**: リプレイ画面の「ライン」ボタンから斜め(ドラッグ)・縦・横(タップ)の線を描ける。色切替・全消去対応。線はlocalStorageに保存され、カメラ固定前提でライブプレビューと全クリップに同じ線が重畳表示される(シャフトプレーン・Vゾーンの手動版として使える)
- ✅ **リプレイ画面での再生速度切替**(0.25/0.5/0.75/1.0)
- 未実装: シャフトプレーン自動検出(OpenCV.js)、円描画、2動画同期比較。[REQUIREMENTS.md](REQUIREMENTS.md) 参照。
